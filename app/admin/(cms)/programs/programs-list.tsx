...
export function ProgramsList({ rows: initial }: { rows: Program[] }) {
  const [rows, setRows] = useState(initial)
  const [query, setQuery] = useState('')

  const filtered = query
    ? rows.filter((r) => r.title.toLowerCase().includes(query.toLowerCase()) || r.category.toLowerCase().includes(query.toLowerCase()))
    : rows

  async function togglePublished(row: Program) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">{rows.length} {rows.length === 1 ? 'programme' : 'programmes'}</p>
        </div>
        <Link
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="mt-4 text-sm font-semibold text-gray-900">
              {query ? 'No programmes match your search' : 'No programmes yet'}
            </h3>
            <p className="mt-1 text-sm text-gray-500">
              {query ? 'Try a different search.' : "Click 'New Programme' to get started."}
            </p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map((row, i) => (
                  <tr key={row.id} className={`${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-indigo-50/30 transition-colors`}>
                    <td className="px-6 py-3.5 font-medium text-gray-900 max-w-xs truncate">{row.title}</td>
                    <td className="px-6 py-3.5 text-gray-600">{row.category}</td>
                    <td className="px-6 py-3.5">
                      <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${LEVEL_COLORS[row.level ?? ''] ?? 'bg-gray-100 text-gray-600'}`}>
                        {row.level}
                      </span>
                    </td>
                    <td className="px-6 py-3.5 text-gray-600 whitespace-nowrap">{row.duration}</td>
                    <td className="px-6 py-3.5">
                      <button
