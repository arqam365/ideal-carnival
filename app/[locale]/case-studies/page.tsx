...
type CaseStudy = {
  id: string; sector: string; institution: string; headline: string
  challenge: string; assessment: string; strategy: string
  implementation: string; transformation: string; results: string[]; impact: string
}

const caseStudies: CaseStudy[] = [
  {
    id: 'ministry-protocol-transformation', sector: 'Government',
    ...
    implementation: "A five-day residential programme was delivered in two cohorts. Faculty included a former Saudi diplomatic protocol officer and a senior EHL hospitality professional. All participants completed scenario simulations of live delegation arrivals, formal receptions, and state luncheons.",
  ...
  {
    id: 'luxury-hotel-service-culture', sector: 'Hospitality',
  ...
  {
    id: 'military-ceremonial-excellence', sector: 'Defense',
    ...
    <main>
              ...
              >
                <div className="grid gap-0 lg:grid-cols-12">
                  ...
                  <div className="grid gap-4 p-8 lg:col-span-9 lg:gap-8 lg:p-10">
                    <div>
                      <span className="text-[0.68rem] font-semibold uppercase tracking-luxury text-gold lg:hidden">{cs.sector} · </span>
                      <span className="text-[0.68rem] font-semibold uppercase tracking-luxury text-muted-foreground">{cs.institution}</span>
                    </div>
                    <div className="flex items-start justify-between gap-4">
                      <h3 className="font-heading text-xl font-medium text-primary transition-colors group-hover:text-primary lg:text-2xl">{cs.headline}</h3>
                      <ArrowRight className="mt-1 size-5 shrink-0 text-muted-foreground transition-all group-hover:translate-x-1 group-hover:text-gold rtl:rotate-180 rtl:group-hover:-translate-x-1 rtl:group-hover:translate-x-0" />
                    </div>
                    <p className="line-clamp-2 text-sm leading-relaxed text-muted-foreground">{cs.challenge}</p>
                  </div>
                </div>
              </a>
            ))}
      ...
      {caseStudies.map((cs, idx) => (
        <section key={cs.id} id={cs.id} className="scroll-mt-24 border-t border-border py-24 lg:py-32">
          <div className="mx-auto max-w-7xl px-6 lg:px-10">
            <div className="reveal mb-14">
              <span className="font-mono text-sm text-gold">{t('caseLabel')} {String(idx + 1).padStart(2, '0')}</span>
              <div className="mt-3 flex flex-wrap items-center gap-3">
                <span className="text-[0.72rem] font-semibold uppercase tracking-luxury text-muted-foreground">{cs.sector}</span>
                <span className="h-3 w-px bg-border" aria-hidden="true" />
                <span className="text-[0.72rem] font-semibold uppercase tracking-luxury text-muted-foreground">{cs.institution}</span>
              </div>
              <h2 className="mt-5 max-w-4xl text-pretty font-heading text-3xl font-medium text-primary sm:text-4xl lg:text-[2.5rem]">{cs.headline}</h2>
            </div>
            <div className="grid gap-12 lg:grid-cols-12">
              <div className="space-y-10 lg:col-span-8">
                <CaseBlock label={t('challengeLabel')} body={cs.challenge} />
                <CaseBlock label={t('assessmentLabel')} body={cs.assessment} />
                <CaseBlock label={t('strategyLabel')} body={cs.strategy} />
                <CaseBlock label={t('implementationLabel')} body={cs.implementation} />
                <CaseBlock label={t('transformationLabel')} body={cs.transformation} />
              </div>
              <aside className="lg:col-span-4">
                <div className="sticky top-28 space-y-8">
                  <div className="border border-gold/30 bg-primary p-8">
                    <Eyebrow light>{tCommon('measuredResults')}</Eyebrow>
                    <ul className="mt-6 space-y-4">
                      {cs.results.map((r) => (
                        <li key={r} className="flex items-start gap-3 text-sm leading-relaxed text-primary-foreground/80">
                  ...
                  <div className="border border-border bg-card p-8">
                    <p className="text-[0.72rem] font-semibold uppercase tracking-luxury text-gold">{tCommon('lastingImpact')}</p>
                    <p className="mt-4 text-sm leading-relaxed text-foreground/80">{cs.impact}</p>
                  </div>
                </div>
              </aside>
            </div>
...
function CaseBlock({ label, body }: { label: string; body: string }) {
  return (
    <div className="border-s-2 border-sand ps-7">
      <p className="text-[0.72rem] font-semibold uppercase tracking-luxury text-muted-foreground">{label}</p>
